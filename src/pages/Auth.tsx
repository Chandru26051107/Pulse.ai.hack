...
function Auth({ redirectAfterAuth }: AuthProps = {}) {
  ...
  const hasAccount = typeof window !== "undefined" && !!localStorage.getItem("pulseflow_session");
  ...
  const handleOAuth = async (provider: "google" | "microsoft") => {
    ...
    if (mode === "signup") {
      // Store new account
      const accounts = JSON.parse(localStorage.getItem("pulseflow_accounts") || "{}");
      accounts[emailAddr] = { name, provider, role: "Viewer", ts: Date.now() };
      localStorage.setItem("pulseflow_accounts", JSON.stringify(accounts));
    }
  ...
  const handleGuest = async () => {
    ...
    localStorage.setItem(
      "pulseflow_session",
      JSON.stringify({ email: "guest@pulseflow.ai", name: "Guest User", role: "Viewer", ts: Date.now(), guest: true }),
    );
    try {
      await signIn("anonymous");
    } catch { /* continue */ }
            ...
            <CardHeader className="text-center pb-2">
              ...
              <CardDescription>
                {mode === "signin"
                  ...
                  : "Sign up to get started with pulseflow.ai"}
              </CardDescription>
            </CardHeader>
